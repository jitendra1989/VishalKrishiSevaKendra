class OrderPdf < Prawn::Document
	def initialize(order, view)
		...
		text_box "In case of cancellation of order, advance", at: [0, 265 - (20 * order.items.size)], size: 10
		text_box "received is non refundable.", at: [0, 250 - (20 * order.items.size)], size: 10
		text_box "Excise Duty, VAT, service tax will be charged extra", at: [0, 235 - (20 * order.items.size)], size: 10
		text_box "if and as is applicable at the time of delivery.", at: [0, 220 - (20 * order.items.size)], size: 10
		text_box "Fabric, Polish shade, design and dimension will not be changed once the order is confirmed.", at: [0, 205 - (20 * order.items.size)], size: 10
		text_box "All the disputes are subjected to Panjim jurisdiction.", at: [0, 190 - (20 * order.items.size)], size: 10
		text_box "Above terms and conditions are approved by me.", at: [0, 175 - (20 * order.items.size)], size: 10
		text_box "Client Signature", style: :bold, at: [0, 80 - (20 * order.items.size)]
	end
